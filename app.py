...
# Cap native BLAS/OpenMP thread counts BEFORE numpy/scikit-learn are
# imported anywhere in the process. Hosting containers (e.g. Streamlit
# Community Cloud's free tier) enforce tight CPU/shared-memory limits;
# NumPy's underlying BLAS library and scikit-learn's joblib backend will
# otherwise try to spawn one native thread/process per detected CPU
# core, which is a known cause of segmentation faults in that kind of
# constrained container. Single-threaded execution is slightly slower
# but this dataset (1,470 rows) trains in a couple of seconds regardless.
